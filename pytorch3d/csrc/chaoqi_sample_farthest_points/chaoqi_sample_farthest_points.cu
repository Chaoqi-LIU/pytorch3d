...
__global__ void ChaoqiFarthestPointSamplingKernel(
    // clang-format off
...
at::Tensor ChaoqiFarthestPointSamplingCuda(
  ...
  at::TensorArg p_t{points, "points", 1}, lengths_t{lengths, "lengths", 2}, 
    r_t{R, "R", 3}, k_t{K, "K", 4}, start_idxs_t{start_idxs, "start_idxs", 5};
  at::CheckedFrom c = "ChaoqiFarthestPointSamplingCuda";
  at::checkAllSameGPU(c, {p_t, lengths_t, r_t, k_t, start_idxs_t});
  ...
  TORCH_CHECK(
      points.size(0) == R.size(0),
      "Points and R must have the same batch dimension");
  ...
  switch (threads) {
    case 1024:
      ChaoqiFarthestPointSamplingKernel<1024>
          <<<blocks, threads, shared_mem, stream>>>(
              points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 512:
      ChaoqiFarthestPointSamplingKernel<512><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 256:
      ChaoqiFarthestPointSamplingKernel<256><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 128:
      ChaoqiFarthestPointSamplingKernel<128><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 64:
      ChaoqiFarthestPointSamplingKernel<64><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 32:
      ChaoqiFarthestPointSamplingKernel<32><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 16:
      ChaoqiFarthestPointSamplingKernel<16><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 8:
      ChaoqiFarthestPointSamplingKernel<8><<<blocks, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 4:
      ChaoqiFarthestPointSamplingKernel<4><<<threads, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    case 2:
      ChaoqiFarthestPointSamplingKernel<2><<<threads, threads, shared_mem, stream>>>(
          points_a, lengths_a, R_a, K_a, idxs_a, min_point_dist_a, start_idxs_a);
      break;
    default:
      ChaoqiFarthestPointSamplingKernel<1024>
